...
function UsablePylons()
	--hier wird gepr�ft, ob die ausgew�hlten Pylons auch die Waffen tragen, 
	--die mit den verschiedenen Selector-Switches ausgew�hlt wurden.
...
function post_initialize()
	...
	-----------------------------------------------------------------------------------
	--Das hier ist GOLD, da es ALLE aktuellen Params anzeigt mit Wert und Ver�nderung--
...
--gunpipper_auto_movement_updown		= gunpipper_updown_automatic_param:get()
	...
	--[[if ir_missile_az_param:get() > 0.0 then
		...
		-- radar is provid�ng target position, but it no lock was achieved yet
...
function keys_pickle_on(value)
	...
	if (station_1 == 1) and
		...
		--dev:launch_station(3) --damit schmei�t er immer eine vom Rack
		Salvo(3)
...
function UsablePylons()
	--hier wird gepr�ft, ob die ausgew�hlten Pylons auch die Waffen tragen, 
	--die mit den verschiedenen Selector-Switches ausgew�hlt wurden.
	--Es soll also gepr�ft werden, ob alle switches (und wie) aktiviert sind. 
	--Z.B. w�rde Rockets mit Station 2 und 4 nicht zum Abwurf von Bomben auf Station 2 und 4 f�hren(hoffentlich).
...
function Salvo(st)
	...
	--nach den "gez�hlten" dropps soll mit return die Funktion verlassen werden, wobei "ohne" return so lnge 
